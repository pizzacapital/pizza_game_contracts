...
import "./Upgrade.sol";
import "./Pizza.sol";
import "./PizzeriaProgressionV3.sol";
import "./PizzeriaV2.sol";

contract PizzeriaV3 is PizzeriaProgressionV3, ReentrancyGuard {
    ...
    uint256 public yieldPPS = 16666666666666667; // pizza cooked per second per unit of yield
    ...
    constructor(Chef _chef, Upgrade _upgrade, Pizza _pizza, Soda _soda, address _freezerAddress) PizzeriaProgressionV3 (_soda) {
        chef = _chef;
        upgrade = _upgrade;
        pizza = _pizza;
    ...
    function getPizzaAccruedForManyChefs(uint256[] calldata _tokenIds) public view returns (uint256[] memory) {
        ...
        for (uint256 i = 0; i < _tokenIds.length; i++) {
            output[i] = getPizzaAccruedForChef(_tokenIds[i], false);
        }
        return output;
    }
    ...
    function getPizzaAccruedForChef(uint256 _tokenId, bool checkOwnership) public view returns (uint256) {
        StakedChef memory stakedChef = stakedChefs[_tokenId];
        ...
        return pizzaAccruedCalculation(chefPizza[_tokenId], delta, ppm, masterChefSkillModifier, chefFatigueLastUpdate, getFatiguePerMinuteWithModifier(owner), yieldPPS);
    }
    ...
    function _updateState(address _owner) internal {
        ...
        for (uint256 i = 0; i < chefBalance; i++) {
            ...
            if (stakedChef.staked && block.timestamp > stakedChef.startTimestamp) {
                chefPizza[tokenId] = getPizzaAccruedForChef(tokenId, false);
    ...
    function _claimPizza(address _owner) internal {
        ...
        for (uint256 i = 0; i < chefBalance; i++) {
            ...
            totalClaimed += getPizzaAccruedForChef(chefId, true); // also checks that msg.sender owns this token
    ...
    function stakeMany(uint256[] calldata _chefIds, uint256[] calldata _upgradeIds) public nonReentrant whenNotPaused {
        ...
        _claimPizza(owner); // Fix bug for incorrect time for upgrades
    ...
    function batchedStakesOfOwner(
        ...
        for (uint256 i = 0; i < outputSize; i++) {
            ...
            outputs[i] = StakedChefInfo({
                ...
                pizza: getPizzaAccruedForChef(chefId, false),
    ...
    function setOldPizzeria(address _oldPizzeria) external onlyOwner {
        oldPizzeria = PizzeriaV2(_oldPizzeria);
    }
